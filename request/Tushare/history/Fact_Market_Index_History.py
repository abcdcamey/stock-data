...
def cal_history_func_date(_date_str,_ts_code,i):
    Logger_process.log('call Fact_Market_Index_History by date,%s,%s' % (_date_str,str(i)))
    print('call Fact_Market_Index_History by date,%s,%s'%(_date_str,str(i)))
...
for i in range(0,7300):
    ...
    for _ts_code in ts_code_list:
        cal_history_func_date(date_str,_ts_code,i)
        time.sleep(1.5)